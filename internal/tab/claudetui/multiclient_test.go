...
// TestGridModeRoleEvent verifies that the grid-mode attachment also delivers
// a role event after grid.init.  The test uses ?mode=grid.
// [AC-S0fd64b-3-1] (grid mode)
func TestGridModeRoleEvent(t *testing.T) {
	bin := fakeBin(t)
	...
	// Wait for some emulator content so grid.init is non-trivial.
	...
	// We expect: grid.init text frame, then role text frame (in that order, but
	// we accept them in any order within 5 s).
	gotInit := false
	gotRole := false
	...
	for !gotInit || !gotRole {
		select {
		case <-deadline2:
			t.Fatalf("timed out: gotInit=%v gotRole=%v", gotInit, gotRole)
		default:
		}
		readCtx, readCancel := context.WithTimeout(ctx, 500*time.Millisecond)
		...
		case "role":
			...
			if jErr := json.Unmarshal(raw, &ev); jErr == nil {
				if ev.Role != RoleActive {
					t.Errorf("grid mode first client: expected active, got %q", ev.Role)
				}
				gotRole = true
			}
		}
	}
	t.Logf("grid mode role event received: gotInit=%v gotRole=%v", gotInit, gotRole)
}
