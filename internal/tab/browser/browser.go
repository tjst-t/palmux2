...
type Manager struct {
	rt          runtime.Runtime // incus runtime for this workspace
...
// NewManager returns a Manager for the given incus runtime.
// inst is the incus instance name (used for bind-mount device naming).
// da may be nil (DefaultDeviceAdder is used).
func NewManager(rt runtime.Runtime, inst string, da DeviceAdder, log *slog.Logger) *Manager {
	if log == nil {
		log = slog.Default()
	}
	if da == nil {
...
func (m *Manager) Start(ctx context.Context) (StartResponse, error) {
	...
	// Resolve container bridge IP from runtime status.
	status := m.rt.Status()
	addr := status.Address
	if addr == "" {
		m.mu.Lock()
...
func (m *Manager) Stop(ctx context.Context) (StopResponse, error) {
	...
	if pid != "" {
		// kill <pid> inside the container — same pattern as UnexposePort relay kill.
		res, err := m.rt.Exec(ctx, []string{"kill", pid}, runtime.ExecOpts{})
		if err != nil {
			m.log.Warn("browser Stop: kill exec error", "inst", m.inst, "pid", pid, "err", err)
		} else if res.ExitCode != 0 && !strings.Contains(res.Stderr, "No such process") {
			m.log.Warn("browser Stop: kill non-zero exit", "inst", m.inst, "pid", pid, "exit", res.ExitCode, "stderr", res.Stderr)
		}
	}
...
func (m *Manager) launchChromium(ctx context.Context, containerIP, profileDir string) (string, error) {
	...
	res, err := m.rt.Exec(ctx, []string{"sh", "-c", launchCmd}, runtime.ExecOpts{})
	if err != nil {
...
func (m *Manager) isPIDAlive(ctx context.Context, pid string) (bool, error) {
	res, err := m.rt.Exec(ctx, []string{"kill", "-0", pid}, runtime.ExecOpts{})
	if err != nil {
		return false, err
	}
