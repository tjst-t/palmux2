...
if [ "$CADDY_ENABLED" = "1" ]; then
  ...
  # --- Sa53137: unified master config (user-owned) ---------------------------
  # In addition to the legacy root-owned runtime.env (kept for back-compat with
  # an already-running unit), write the new user-owned master config.toml plus
  # secrets.env (0600). palmux's resolution chain reads these (flag > env >
  # file > default), and the GUI deploy tab / `palmux apply` edit them. This is
  # what lets domain/secret changes happen without a home-manager switch.
  PALMUX_CONFIG_DIR="${USER_HOME}/.config/palmux"
  log "writing ${PALMUX_CONFIG_DIR}/config.toml + secrets.env (user-owned 0600)"
  install -d -m 0755 "$PALMUX_CONFIG_DIR" 2>/dev/null || mkdir -p "$PALMUX_CONFIG_DIR"
  {
    echo "[server]"
    echo "addr = \"127.0.0.1:8080\""
    echo "caddy_admin = \"http://localhost:2019\""
    echo ""
    echo "[public]"
    echo "domain = \"${DOMAIN}\""
    if [ -n "$BASIC_AUTH_USER" ]; then
  ...
  } > "${PALMUX_CONFIG_DIR}/config.toml"
  chmod 0600 "${PALMUX_CONFIG_DIR}/config.toml"
  {
    echo "# palmux secrets (user-owned, 0600). Generated by install.sh."
    echo "PALMUX_SSO_SECRET=${PALMUX_SSO_SECRET}"
    if [ -n "$BASIC_AUTH_HASH" ]; then
      echo "BASIC_AUTH_HASH=${BASIC_AUTH_HASH}"
    fi
  } > "${PALMUX_CONFIG_DIR}/secrets.env"
  chmod 0600 "${PALMUX_CONFIG_DIR}/secrets.env"

  # --- Sa53137-4-3: sudoers drop-in for the single privileged reconcile verb -
  # Grant the install user NOPASSWD for EXACTLY `palmux2 reconcile-system` (and
  # nothing else). The verb takes no free-form input: it reads the user-owned
  # config.toml, strictly validates the domain, renders /etc/caddy/Caddyfile from
  # a fixed template, and reloads caddy. No arbitrary command/path is accepted.
  PALMUX_BIN_FOR_SUDOERS="$(command -v palmux2 || echo "${USER_HOME}/.nix-profile/bin/palmux2")"
  log "installing sudoers drop-in for reconcile-system (NOPASSWD, verb-limited)"
  {
    echo "# Managed by palmux install.sh (Sa53137-4). Allows the single privileged"
    echo "# verb 'palmux2 reconcile-system' without a password. No other command."
    echo "${USERNAME} ALL=(root) NOPASSWD: ${PALMUX_BIN_FOR_SUDOERS} reconcile-system, ${PALMUX_BIN_FOR_SUDOERS} reconcile-system *"
  } | sudo tee /etc/sudoers.d/palmux-reconcile >/dev/null
  sudo chmod 0440 /etc/sudoers.d/palmux-reconcile
  # Validate the drop-in; remove it if it doesn't parse (never leave a broken
  # sudoers file that could lock the user out).
  if ! sudo visudo -cf /etc/sudoers.d/palmux-reconcile >/dev/null 2>&1; then
    log "WARNING: sudoers drop-in failed validation; removing it"
    sudo rm -f /etc/sudoers.d/palmux-reconcile
  fi
