#!/usr/bin/env bash
# Sprint S13b16a regression suite runner.
# Inherits the S4b9df4 master suite (22 E2E tests + go test + go build + npm
# build + npm lint) but always runs all 22 tests (INCLUDE_NEW is implicit).
# Outputs per-test logs into docs/sprint-logs/S13b16a/${OUT_TAG}-stdout/
# then assembles JSON summary at docs/sprint-logs/S13b16a/regression-${OUT_TAG}.json.
#
# Usage:  PORT=8200 OUT_TAG=baseline ./scripts/sprint-regression-S13b16a.sh
set -uo pipefail

PORT="${PORT:-8200}"
OUT_TAG="${OUT_TAG:-baseline}"
LOG_DIR="docs/sprint-logs/S13b16a/${OUT_TAG}-stdout"
mkdir -p "$LOG_DIR"

# Same trick as S4b9df4: legacy tests hardcode pre-S1e8d02 branch IDs that no
# longer exist on this dev rig. Override to a real open branch on the test
# repo so attach succeeds.
...
summary_json="docs/sprint-logs/S13b16a/regression-${OUT_TAG}.json"
