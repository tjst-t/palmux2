#!/usr/bin/env bash
# Sprint Saa8506 regression suite runner.
# Master Regression Suite: go test + go build + npm build + npm lint +
# 23 E2E tests (22 pre-Saa8506 + 1 new s006_attach_dnd_wire from
# Saa8506-2). Outputs per-test logs into
# docs/sprint-logs/Saa8506/${OUT_TAG}-stdout/ then assembles JSON summary
# at docs/sprint-logs/Saa8506/regression-${OUT_TAG}.json.
#
# Saa8506-3: the env override layer (auto-export S00._BRANCH_ID for every
# legacy test) has been removed. All 23 tests are hermetic — each test
# creates its own ghq repo + opens it via the palmux2 API, then tears it
# down on exit. There is nothing to bridge between the script and the
# tests beyond PALMUX2_DEV_PORT.
#
# Usage:  PORT=8200 OUT_TAG=baseline ./scripts/sprint-regression-Saa8506.sh
set -uo pipefail

PORT="${PORT:-8200}"
OUT_TAG="${OUT_TAG:-baseline}"
LOG_DIR="docs/sprint-logs/Saa8506/${OUT_TAG}-stdout"
mkdir -p "$LOG_DIR"

echo "==> Hermetic regression: each test owns its own fixture repo."
...
# Phase 1: backend go test (claudeagent only, matching S4b9df4 scope)
...
# Extract lint counts from the eslint summary line "✖ N problems (E errors, W warnings)".
# Must look at the summary line specifically — the "potentially fixable" line that
# follows also matches "N errors and M warnings" with different numbers.
fe_lint_summary=$(grep -E '^\s*✖\s+[0-9]+\s+problems' "$fe_lint_log" | tail -1 || true)
fe_lint_errors=$(echo "$fe_lint_summary" | grep -oE '[0-9]+ errors?' | head -1 | grep -oE '[0-9]+' || echo "")
fe_lint_warnings=$(echo "$fe_lint_summary" | grep -oE '[0-9]+ warnings?' | head -1 | grep -oE '[0-9]+' || echo "")
[ -z "$fe_lint_errors" ] && fe_lint_errors=0
[ -z "$fe_lint_warnings" ] && fe_lint_warnings=0
...
summary_json="docs/sprint-logs/Saa8506/regression-${OUT_TAG}.json"
...
cat > "$summary_json" <<EOF
{
  ...
  "phases": {
    ...
    "fe_lint": {"result": "$fe_lint_result", "exit": $fe_lint_status, "duration_s": $fe_lint_dur, "log": "$fe_lint_log", "errors": $fe_lint_errors, "warnings": $fe_lint_warnings}
  },
...
echo "  go_test: $go_test_result | go_build: $go_build_result | fe_build: $fe_build_result | fe_lint: $fe_lint_result (errors=$fe_lint_errors warnings=$fe_lint_warnings)"
