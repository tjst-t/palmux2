...
def test_export_json_download(page) -> None:
    ...
    # The filename is regenerated by a useEffect on [format, branchId, open],
    # so the input value updates one render after the click. Wait for it.
    page.wait_for_function(
        "() => (document.querySelector(\"[data-testid='export-filename']\")?.value || '').endsWith('.json')",
        timeout=2000,
    )
    fname = page.locator("[data-testid='export-filename']").input_value()
