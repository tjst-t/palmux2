...
## スプリント S015: Worktree categorization (my / unmanaged / subagent) [ ]
...
### ストーリー S015-1: Drawer 上で worktree を 3 カテゴリに分けて表示し、 既存 worktree を Palmux2 管理下に明示的に移せる [ ]
...
**カテゴリ分類:**
- [ ] Drawer は 3 セクション (my / unmanaged / subagent) に分かれて表示される
- [ ] ユーザが Drawer から open した (= gwq add 経由) ブランチは自動的に `user_opened_branches` に追加され、 my セクションに入る
- [ ] `git worktree add` を CLI 直接実行した worktree は unmanaged セクションに表示される
- [ ] パスが `autoWorktreePathPatterns` (デフォルト `.claude/worktrees/*`) に合致する worktree は subagent セクションに表示される
- [ ] グローバル設定で `autoWorktreePathPatterns` を編集すると分類が更新される (ページリロードで反映)

**表示優先度:**
- [ ] my セクションは最上部、 デフォルト展開
- [ ] unmanaged セクションは my の直後、 デフォルト展開 (既存を「優先表示」)
- [ ] subagent セクションは最下部、 デフォルト折りたたみ、 件数 badge をヘッダに表示
- [ ] subagent セクションの各エントリには 🤖 アイコン + muted 色で「自動」を視覚的に明示
- [ ] セクションの折りたたみ状態は `localStorage` に保存 (`palmux:drawer.section.<key>.collapsed`)

**Promote action:**
- [ ] unmanaged セクションの各行に `+ Add to my worktrees` ボタンが表示される
- [ ] クリックで `POST /api/repos/{repoId}/branches/{branchId}/promote` が呼ばれ、 当該ブランチが my セクションに移動する (楽観的更新、 失敗時 revert)
- [ ] my セクションには promote ボタンは出ない (既に管理下)
- [ ] subagent セクションには promote ボタンを出さない (本 sprint スコープ外、 backlog で別途対応)

**reconcile:**
- [ ] palmux2 起動時に `user_opened_branches` の各エントリのパスが実在するかチェックし、 失われていれば自動削除
- [ ] CLI で直接 `gwq remove` した後でも次回起動時に my から消える

**モバイル:**
- [ ] モバイル (< 600px) で 3 セクションすべてが折りたたみ可
- [ ] `+ Add to my worktrees` ボタンのタップ領域が指で押せるサイズ (~36px)

**タスク:**

**バックエンド:**
- [ ] **タスク S015-1-1**: `internal/config/repos.go` (or 該当箇所) の `repos.json` schema を拡張: 各 repo entry に `user_opened_branches []string` フィールドを追加 (`omitempty`)。 既存ファイル読み込み時に nil なら空 slice として扱う migration 互換層
- [ ] **タスク S015-1-2**: グローバル設定 `autoWorktreePathPatterns []string` を追加 (デフォルト `[".claude/worktrees/*"]`)。 `internal/config/settings.go` 等。 `GET/PATCH /api/settings` で読み書き可
- [ ] **タスク S015-1-3**: `POST /api/repos/{repoId}/branches/{branchId}/promote` を追加 — `user_opened_branches` に追加 (重複チェック)、 atomic write で `repos.json` に反映、 WS event `branch.categoryChanged` を emit
- [ ] **タスク S015-1-4**: `DELETE /api/repos/{repoId}/branches/{branchId}/promote` を追加 — `user_opened_branches` から削除 (worktree 自体は残す)。 万一の対称性確保 (UI からは subagent への demote 等で使う)
- [ ] **タスク S015-1-5**: ブランチ list を返す既存 API (`GET /api/repos/{repoId}/branches` 相当) のレスポンスに各 branch の `category: 'user' | 'unmanaged' | 'subagent'` フィールドを追加。 検出ロジック:
  ```
  if branch in user_opened_branches → 'user'
  elif worktree.path matches any autoWorktreePathPatterns (glob) → 'subagent'
  else → 'unmanaged'
  ```
- [ ] **タスク S015-1-6**: ユーザが Drawer から「Open branch」した既存経路 (`gwq add` 呼び出し) で、 成功後に `user_opened_branches` に当該 branch 名を append する。 既存ハンドラ (おそらく `internal/server/...` の workspace 系) を更新
- [ ] **タスク S015-1-7**: 起動時 reconcile: `user_opened_branches` の各エントリについて `git worktree list` で path が実在するか確認、 失われていれば slice から除去して `repos.json` に書き戻す。 panic-safe (1 repo の reconcile 失敗で全体を止めない)

**フロントエンド:**
- [ ] **タスク S015-1-8**: Drawer (`frontend/src/components/drawer.tsx`) を 3 セクション構造に refactor。 my / unmanaged / subagent それぞれを `<DrawerSection>` 共通コンポーネントで描画
- [ ] **タスク S015-1-9**: セクションの折りたたみ状態を `localStorage` (`palmux:drawer.section.my.collapsed` 等) に保存・復元する hook を追加。 デフォルトは my=expanded、 unmanaged=expanded、 subagent=collapsed
- [ ] **タスク S015-1-10**: subagent セクションのヘッダに件数 badge (例: `subagent / autopilot (3)`)。 折りたたみ状態でも見える位置に
- [ ] **タスク S015-1-11**: subagent セクションの各エントリに 🤖 アイコン + muted 色 (`var(--color-fg-muted)`) を適用
- [ ] **タスク S015-1-12**: unmanaged セクションの各行に `+ Add to my worktrees` ボタンを追加。 クリックで `POST /api/.../promote` を呼び、 楽観的更新で my セクションに移動。 失敗時は revert + Toast でエラー表示
- [ ] **タスク S015-1-13**: WS event `branch.categoryChanged` を listen して、 別クライアントでの promote 操作も即時反映
- [ ] **タスク S015-1-14**: モバイル (< 600px) でセクション折りたたみとボタンタッチを最適化。 `+` ボタンのタップ領域 36px 以上

**E2E:**
- [ ] **タスク S015-1-15**: dev インスタンス + Playwright で実機検証。`tests/e2e/s015_*.py` で:
...
## スプリント S017: Long session performance (virtualization + Read プレビュー) [ ]
...
### ストーリー S017-1: 長 Claude セッションでも軽快に動作する [ ]
...
**受け入れ条件:**
- [ ] 500 ターンの会話を開いてもスクロールが滑らか (60fps 維持)
- [ ] 折りたたみ展開時にレイアウト崩れなし、 サイズ再計算がアニメーションせずに一発で決まる
- [ ] Read tool が大ファイル (1000 行+) を読んだとき、 デフォルトで先頭 50 行 + 「Show all (X lines)」 ボタン表示
- [ ] 「Show all」 で全展開、 折りたたみ可能、 再度プレビューに戻すこともできる
- [ ] 設定 `readPreviewLineCount` を変えると即座に反映される
- [ ] モバイル (< 600px) でも仮想化が動作、 タッチスクロールが滑らか
- [ ] Scroll position は session reload 後も復元される (URL or localStorage 保存)

**タスク:**
- [ ] **タスク S017-1-1**: `react-window` を導入 (`npm i react-window`)、 既存の Claude タブの会話描画を `VariableSizeList` 化
- [ ] **タスク S017-1-2**: 各 turn / block のサイズ計測ロジック (展開状態 / コード行数 / 画像有無を考慮)
- [ ] **タスク S017-1-3**: 折りたたみトグル時の `resetAfterIndex` 呼び出し + smooth scroll preservation
- [ ] **タスク S017-1-4**: Scroll position の永続化 (`localStorage` or URL hash、 session id keyed)
- [ ] **タスク S017-1-5**: Read tool result のプレビュー切り出しロジック (先頭 N 行)、 BE 側で `lines: { preview, total }` を返すか、 FE 側で文字列を分割するかの判断 → 推奨: FE 側で分割 (BE 既存形式維持)
- [ ] **タスク S017-1-6**: 「Show all (X lines)」 ボタン UI、 状態管理 (preview / expanded)
- [ ] **タスク S017-1-7**: 設定 `readPreviewLineCount` を追加 (`internal/config/settings.go`、 GET/PATCH /api/settings)
- [ ] **タスク S017-1-8**: モバイルで仮想化動作の確認 + デザイン調整
- [ ] **タスク S017-1-9**: dev インスタンス + Playwright E2E (`tests/e2e/s017_*.py`): (a) 500 ターン synthetic セッション生成 → スクロール、 (b) 1000 行 Read 結果 → preview 表示 + Show all 動作、 (c) 設定変更で反映、 (d) モバイル幅、 (e) 折りたたみ + 再展開でレイアウト崩れなし、 (f) reload で scroll position 復元

---

## スプリント S018: Conversation utilities (search + export + /compact) [ ]
...
### ストーリー S018-1: 会話を探す・残す・圧縮する操作を Palmux 内で完結できる [ ]
...
**会話内検索:**
- [ ] `Cmd+F` (Mac) / `Ctrl+F` (Windows/Linux) で会話内検索バーが現れる (ブラウザの Cmd+F は呼ばない)
- [ ] 検索クエリ入力でマッチ件数 (例: `3/12`) が表示される
- [ ] マッチを含む折りたたまれたブロックは自動展開、 マッチ箇所がハイライトされる
- [ ] Enter / Shift+Enter で次/前のマッチへ scroll into view
- [ ] Escape で検索バーを閉じる

**エクスポート:**
- [ ] Claude タブヘッダに「Export」 ボタン
- [ ] クリックで形式選択 dialog (Markdown / JSON)、 ファイル名 input (デフォルト: `<branch>-<date>.md` 等)
- [ ] Markdown 形式: 各 turn が `## User` / `## Assistant` で整形、 tool use/result は `<details>` で埋め込み、 そのまま Slack や issue に貼れる
- [ ] JSON 形式: 生 stream-json envelope を dump (将来 import で再生できる素材)
- [ ] Download 後にファイルが保存される

**`/compact`:**
- [ ] composer で `/` を入力すると slash menu に `compact` が出る
- [ ] 選択で確認 dialog (「過去の会話を要約します。 圧縮した内容は失われます。 続行しますか？」)
- [ ] 確認で CLI に control_request 送信、 進行中 spinner
- [ ] 完了で過去ターンが「Compacted: X turns into 1 summary」 ブロック (要約内容含む) に置換され、 以降の応答が早くなる

**モバイル:**
- [ ] 検索バー、 Export ダイアログ、 slash menu すべてがモバイル幅で動作

**タスク:**
- [ ] **タスク S018-1-1**: 会話内検索 index 構築 — 各 block の text を indexable に保つ store (Zustand or SWR cache)。 仮想化 (S017) と整合
- [ ] **タスク S018-1-2**: 検索 UI コンポーネント (`<ConversationSearchBar>`)、 `Cmd+F` のキーバインド
- [ ] **タスク S018-1-3**: マッチハイライト + 折りたたみ自動展開 + scroll into view
- [ ] **タスク S018-1-4**: Markdown serializer (`internal/tab/claudeagent/export.go` or FE 側 `lib/conversation-export.ts`、 推奨は FE で session snapshot から)
- [ ] **タスク S018-1-5**: JSON serializer (生 stream-json envelope dump)
- [ ] **タスク S018-1-6**: Export ダイアログ UI (形式選択 + ファイル名 input + download trigger)
- [ ] **タスク S018-1-7**: composer slash menu に `compact` 追加 (既存 slash menu の機構流用)
- [ ] **タスク S018-1-8**: `/compact` の control_request 仕様 spike — 実 CLI を `--include-partial-messages` で叩いて wire format を確認 (S005 hook event の調査と同じパターン)
- [ ] **タスク S018-1-9**: `/compact` BE 統合 (`Agent` に compact 経路を追加、 圧縮済みサマリブロックを生成)
- [ ] **タスク S018-1-10**: モバイル UX (検索バーのキーボード表示、 Export dialog、 slash menu のタッチ対応)
- [ ] **タスク S018-1-11**: dev インスタンス + Playwright E2E (`tests/e2e/s018_*.py`): (a) 検索でマッチ navigation、 (b) Markdown / JSON export、 (c) `/compact` 実行 → サマリブロック表示、 (d) モバイルでの動作

---

## スプリント S019: Conversation rewind (claude.ai-style edit & rewind) [ ]
...
### ストーリー S019-1: 過去の user message を編集してそこから会話をやり直せる [ ]
...
**タスク:**
- [ ] **タスク S019-1-1**: CLI rewind 機能 spike — Claude Code 2.1.123 で `/rewind <count>` の wire format を `--include-partial-messages` で確認、 stream-json での発火経路を特定
- [ ] **タスク S019-1-2**: データモデル拡張 — `Turn.versions: TurnVersion[]` (BE: `internal/tab/claudeagent/session.go`、 FE: `frontend/src/tabs/claude-agent/types.ts`、 `agent-state.ts`)。 `TurnVersion` は `{content: string, createdAt: time, subsequentTurnIds: TurnID[]}` 構造
- [ ] **タスク S019-1-3**: Session truncation API — `POST /api/repos/{repoId}/branches/{branchId}/tabs/claude/sessions/rewind` (`{turn_id, new_message}` 受信、 CLI に rewind 発火 + 新 message 送信、 旧 versions を保持)
- [ ] **タスク S019-1-4**: WS event `session.rewound` emission (新バージョン番号 / 旧バージョン番号 / turn_id を payload)
- [ ] **タスク S019-1-5**: user message に edit pencil アイコン + hover 表示 (Fog palette、 既存 hover パターンに揃える)
- [ ] **タスク S019-1-6**: inline editor 起動 — Monaco editor (S010 流用) を user message の場所にオーバーレイ、 Cmd+Enter / Esc キーバインド
- [ ] **タスク S019-1-7**: 編集中の下書き保持 — `localStorage` に `palmux:rewind-draft.<turnId>` で保存、 編集モード終了で削除
- [ ] **タスク S019-1-8**: version navigation arrows UI (`< N/M >` 形式)、 各 user turn の上に表示
- [ ] **タスク S019-1-9**: arrow クリックで subsequent turns 切替の reducer ロジック (display state のみ、 BE への問い合わせ不要)
- [ ] **タスク S019-1-10**: 楽観的 UI — submit クリック時に subsequent turns を fade-out → 削除、 失敗時 revert
- [ ] **タスク S019-1-11**: モバイル UX — edit pencil のタップ領域 36px+、 編集モードのキーボード表示時のスクロール調整
- [ ] **タスク S019-1-12**: dev インスタンス + Playwright E2E (`tests/e2e/s019_*.py`): (a) 過去 message edit → submit → 再生成、 (b) navigation arrows でバージョン切替、 (c) 編集中の下書き保持 (別タブ切替後も復元)、 (d) WS 同期 (別ブラウザでバージョン切替)、 (e) モバイル幅で動作、 (f) Esc でキャンセル
...
## スプリント S021: Subagent worktree lifecycle (cleanup + promote) [ ]
...
### ストーリー S021-1: 溜まった subagent worktree を一括掃除し、良い結果を恒久化できる [ ]
...
**受け入れ条件:**
- [ ] Drawer の subagent セクションヘッダに「Clean up」ボタン
- [ ] クリックで stale (lock なし + 最終 commit から `subagentStaleAfterDays` 経過、 デフォルト 7 日) な worktree のリストが dialog 表示される
- [ ] 確認で一括削除 (`gwq remove`)、 失敗時はエラー Toast
- [ ] 各 subagent worktree 行に「Promote to my」 ボタン (右クリックメニュー or hover アクション)
- [ ] クリックで worktree が gwq の標準位置に move + my セクションに移動 + autoWorktreePathPatterns との接触解除
- [ ] Move 中は spinner、 完了で Drawer が更新 (WS event)
- [ ] 設定 `subagentStaleAfterDays` で stale 判定の閾値変更可
- [ ] WS event `worktree.cleaned` / `branch.promoted` で別クライアント同期

**タスク:**
- [ ] **タスク S021-1-1**: Stale 判定ロジック (`internal/store/` か `internal/tab/sprint/` か関連箇所): lock ファイル check + worktree の最終 commit 時刻を `git log -1 --format=%cI` で取得 → 経過日数判定
- [ ] **タスク S021-1-2**: `POST /api/repos/{repoId}/worktrees/cleanup-subagent` (`{dry_run, paths?}` 受信、 dry_run で対象 list を返す、 確定で削除)
- [ ] **タスク S021-1-3**: `POST /api/repos/{repoId}/branches/{branchId}/promote-subagent` (gwq 標準位置に move + `user_opened_branches` 追加 + autoWorktreePathPatterns との接触除外)
- [ ] **タスク S021-1-4**: 設定 `subagentStaleAfterDays` を追加 (デフォルト 7)
- [ ] **タスク S021-1-5**: Drawer の subagent セクションヘッダに「Clean up」ボタン + 確認 dialog (一覧表示)
- [ ] **タスク S021-1-6**: subagent 行に「Promote to my」 アクション (hover / 右クリックメニュー) + 確認 dialog
- [ ] **タスク S021-1-7**: WS event `worktree.cleaned` / `branch.promoted` emission + handling
- [ ] **タスク S021-1-8**: dev インスタンス + Playwright E2E (`tests/e2e/s021_*.py`): (a) stale subagent worktree を 3 つ作成 → cleanup ボタン → 削除確認 → 一括削除、 (b) subagent worktree を promote → gwq 標準位置に move + my に移動、 (c) 設定変更で stale 判定閾値が変わる

---

## スプリント S022: Mobile UX 総点検 + Playwright モバイル E2E ハーネス [ ]
...
**受け入れ条件:**
- [ ] 各タブ (Claude / Files / Git / Bash / Sprint) と各 popup (Settings / History / MCP / etc.) と各 dialog (confirm / select / prompt) がすべてモバイル幅 (320px〜599px) で破綻なく表示
- [ ] タップ領域がすべて 36px+ で統一 (CSS variable で管理)
- [ ] Selector / Drawer (モバイル) / popup の表示が **bottom sheet** スタイル (画面下からスライド、 close は drag down or backdrop tap)
- [ ] Touch gesture (S012 swipe / S009 long-press / S010 SVG / S016 Mermaid pinch-zoom 等) が相互干渉せず動作
- [ ] 初期 bundle が dynamic import で分割、 初期ロード < 500KB (gzip 後)

**タスク:**
- [ ] **タスク S022-1-1**: 各タブ・popup・dialog のモバイル監査 — チェックリスト作成 + 改善点抽出 + 個別修正
- [ ] **タスク S022-1-2**: タップ領域サイズ統一 — Fog palette に `--tap-min-size: 36px` を追加、 各 button / link / icon に適用
- [ ] **タスク S022-1-3**: Selector / Drawer / popup の bottom sheet 化 — 共通 `<BottomSheet>` コンポーネントを新設、 モバイル幅で active
- [ ] **タスク S022-1-4**: Touch gesture 干渉解消 — 各 sprint の gesture を整理、 衝突回避ルールをドキュメント化 (`docs/mobile-gestures.md`)
- [ ] **タスク S022-1-5**: Bundle splitting — Vite の dynamic import で Files (Monaco) / Git / Sprint / drawio / Mermaid を lazy load、 初期 bundle < 500KB

### ストーリー S022-2: モバイル UI の自動回帰テストハーネスを整備する [ ]

**ユーザーストーリー:**
開発者として、 モバイル UI の回帰テストを自動化したい。 なぜなら、 各 sprint のモバイル動作確認を手動でやるのは持続不可能で、 既存 sprint (S012-S021) のモバイル機能が後の変更で壊れるリスクが高いからだ。

**受け入れ条件:**
- [ ] `tests/e2e/mobile/*.py` にモバイル専用 E2E ディレクトリ
- [ ] viewport 固定 (375x667 iPhone SE 標準) + touch emulation 設定
- [ ] 既存 sprint の主要機能 (S001 Plan / S007 Ask / S012 Git status / S015 Drawer / S017 long session / etc.) のモバイル E2E が PASS
- [ ] CI 化 (Phase 5 で本格化、 本 sprint は手動実行可能まで)

**タスク:**
- [ ] **タスク S022-2-1**: Playwright モバイル設定 (`tests/e2e/mobile/conftest.py`、 viewport / touch / user-agent)
- [ ] **タスク S022-2-2**: 主要 5-10 シナリオのモバイル variant 化 (S001 Plan、 S007 Ask、 S012 swipe stage、 S015 promote、 S017 仮想化 long session、 etc.)
- [ ] **タスク S022-2-3**: CI 化 spike — GitHub Actions or 自前 cron で実行可能までの skeleton 整備 (本 sprint では本番投入しない、 手動実行可能で完了)
- [ ] **タスク S022-2-4**: 全モバイル E2E が PASS する状態で sprint 完了
