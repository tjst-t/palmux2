...
1. **トークン全面縮小**: font 12px / 11.5px / 9-10px、 padding 5/10/22px (v3 比約 50% 削減)。 240px 幅で repo 名 / branch 名が ellipsis を最小限に表示
2. **Single-expand**: 同時に展開できる repo は 1 つだけ。 別 repo を展開で前のを自動 collapse (アコーディオン挙動)。 state を `expandedRepoId: string | null` 単一 ref に refactor
3. **HERE ラベル削除**: `<HereLabel>` コンポーネント削除、 active branch は CSS のみで識別 (border-left 3px accent + bold + bg tint + 2.6s glow)
4. **Worktree 1 行化**: my branch / sub-branch (chip-expanded) すべて 1 行 layout。 削除する meta: `idle Nm` / `N turns` / `created YYYY` / `stale Nd` / `Nh ago` 等の text。 残す: branch 名 / `⌂` ghq mark / `[main]` badge / `●` (fresh) / `◍` (stale) stat icon
5. **Glance line**: collapsed repo 直下に navigate 先 1 行プレビュー (`⌂ main ›`)。 active を含む repo は accent カラー化
...
- [x] **タスク S024-1-10**: dev インスタンス + Playwright で実機検証。 `tests/e2e/s024_*.py` で:
  ...
  - (d) Worktree 1 行: branch row が縦方向に 1 行 (高さチェック)
  - (e) Glance line: collapsed repo の直下に navigate target が表示
...
## スプリント S025: E2E test fixture cleanup hygiene [x]
...
### ストーリー S025-1: E2E が後始末を残さないようにする [x]
...
**受け入れ条件:**
- [x] 既存の `~/ghq/github.com/palmux2-test/*` を一括削除する script が用意される
- [x] 既存の `tmp/repos.json` の `palmux2-test--*` エントリも削除される (script で同時に or 連動で)
- [x] 共通 cleanup helper (`tests/e2e/_fixture.py` or `conftest.py`) が新設され、 fixture 作成 → テスト → cleanup を context manager / try-finally で確実に実行
- [x] 既存 7 E2E テスト (s015 / s016 / s016_fix1 / s020 / s021 / s023 / s024) が helper を使うように refactor、 fail / 例外時も cleanup が走る
- [x] 各 E2E テスト完走後に `~/ghq/github.com/palmux2-test/` が空、 `repos.json` に `palmux2-test--*` が 0 件
- [x] テスト実行が中断された場合 (Ctrl-C) も基本 cleanup 動作 (signal handler or atexit hook)
- [x] CI / make target からも cleanup script を呼べる

**タスク:**

- [x] **タスク S025-1-1**: 一括 cleanup script (`scripts/cleanup-test-fixtures.py`) — `~/ghq/github.com/palmux2-test/*` を rmtree + `repos.json` の `palmux2-test--*` エントリを削除 (palmux2 の `POST /api/repos/{id}/close` 経由、 server offline なら直接書き換えにフォールバック)
- [x] **タスク S025-1-2**: 既存ゴミを実機で削除 (script を 1 回実行)、 ドロワー / repos.json から palmux2-test--* が消えることを確認 (before: 7 ids + 4 dirs / after: 0 + 0)
- [x] **タスク S025-1-3**: 共通 fixture helper (`tests/e2e/_fixture.py`) — context manager `palmux2_test_fixture(sprint_id)` + dataclass `Fixture` + WeakSet 経由の atexit/SIGINT/SIGTERM hook
- [x] **タスク S025-1-4**: 既存 7 E2E テストを helper 経由に refactor、 try / finally で cleanup を保証 (各テストの `make_fixture_repo()` / `fixture_cleanup()` を helper の thin wrapper に置換)
- [x] **タスク S025-1-5**: signal handler / atexit hook で Ctrl-C 中断時にも cleanup を試行 (SIGINT / SIGTERM 双方を手動検証、 leftover 0)
