...
Palmux は Web ベースのターミナルクライアント。tmux セッションをブラウザから操作する。Go シングルバイナリ（フロントエンド embed）、PC / モバイル両対応。
...
グローバル設定には Toolbar の全モード（normal/shortcut/claude/command）のボタン構成が含まれる。各ボタンは `type`（modifier, key, ctrl-key, arrow, page, popup, fontsize, ime, speech, command）で定義。設定ファイルで省略されたモードはデフォルト値で補完する。claudeMode の rows を編集すればスラッシュコマンドのカスタマイズが可能。
