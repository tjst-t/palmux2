...
| **G-11** | S029-fix-1 | Wheel-zoom + pointer-drag inside Sprint Dependency Graph | Sprint tab → Mermaid SVG container | Apply CSS transform for zoom (cursor-centred) and pan; container uses `touch-action: none` so the page doesn't intercept |

> **Removed in S029**: G-3 / G-4 / G-5 — the swipe-stage / swipe-unstage /
> swipe-discard gestures on Git status rows. They were dropped together with
> the rest of the Git tab advanced GUI (see `docs/sprint-logs/S029/`) and
> moved to the backlog. The redesigned Git tab uses a tap-to-select +
> tap-the-`+`/`−`-icon flow that works the same on touch and pointer.
...
| ↓ vs → | G-1 | G-7 | G-8 | G-9 | G-11 |
|---|---|---|---|---|---|
| **G-1** (long-press tab menu) | — | **G-7 supersedes** if drag begins before menu opens; ContextMenu cancels on `pointermove` past 8 px threshold | — | — | — |
| **G-7** (tab drag-reorder) | see G-1 | — | — | — | — |
| **G-8** (conversation scroll) | — | — | — | — | — |
| **G-9** (sheet drag-down) | — | — | — | — | — |
| **G-11** (graph zoom/pan) | — (different surface) | — | — | — | wheel and drag-pan share the same surface; movement threshold (4 px) distinguishes a click on a node from a pan |
...
- **Click vs pan (G-11)**: Sprint Dependency Graph distinguishes a node click
  from a pan via a 4 px movement threshold. After a confirmed pan, a 250 ms
  click-suppression window prevents the synthetic mouseup-click from
  navigating to whichever node lies under the cursor at release.
...
| Mermaid container (Files SVG viewer) | `pinch-zoom` | Diagram viewer |
| SVG iframe | (sandbox controls touch via attr) | Sandboxed |
| Sprint Dependency Graph viewport | `none` | Wheel-zoom + pointer-drag (G-11) own the gesture; native page scroll is suppressed inside the 600 px viewport |
...
- S022 mobile audit — all gestures fire correctly at 320 / 375 / 599 px viewports.
- Mobile E2E suite (`tests/e2e/mobile/`) covers:
  - G-9 (BottomSheet drag-down)
  - G-1 + G-7 collision (long-press disambiguation)
- S029 — G-3 / G-4 / G-5 (Git swipe-stage / -unstage / -discard) deleted along
  with the Git tab advanced GUI. The replacement minimal Git tab uses
  tap-to-select + tap-`+`/`−` for stage / unstage on touch.
