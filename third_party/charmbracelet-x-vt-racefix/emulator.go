...
type Emulator struct {
	...
	//
	// palmux2 patch (S64c835-1 / docs/sprint-logs/S64c835/decisions.json):
	// this was a plain bool, unsynchronized against concurrent Read()/
	// Close() calls from different goroutines (SafeEmulator does not
	// override Close() at all, and its Read() explicitly bypasses its own
	// mutex — see safe_emulator.go). Read() can block indefinitely inside
	// e.pr.Read(), so a caller-side mutex covering the whole Read() call
	// cannot be shared with Close() without either deadlocking (Close()
	// waits for a lock a blocked Read() holds forever) or leaving a
	// genuine, -race-flagged data race (Close() bypassing that lock to
	// interrupt the blocked call still races the *next* Read() call's
	// check, no matter how long it waits — there is no way to establish a
	// happens-before edge for that case without this field's own
	// synchronization). Making the field atomic fixes this at the root:
	// io.PipeReader.Read and io.PipeWriter.CloseWithError are themselves
	// already safe for concurrent use (that is the entire point of
	// io.Pipe), so once this flag no longer races, Close() vs Read()/Write()
	// is fully race-free with no additional locking and no deadlock risk.
	closed atomic.Bool
